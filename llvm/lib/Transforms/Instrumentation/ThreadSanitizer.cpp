...
// only MemoryAccess event could be ignored
static bool isIgnoreEligible(EventType Eid) {
  switch (Eid) {
  case kEventRead:
  case kEventWrite:
  case kEventMemset:
...
static const char *getEventTypeName(EventType Eid) {
  switch (Eid) {
  case kEventClear: return "Clear";
  case kEventRead: return "Read";
  case kEventWrite: return "Write";
  case kEventVptrUpdate: return "VptrUpdate";
  case kEventVptrLoad: return "VptrLoad";
  case kEventMemset: return "Memset";
  case kEventMemcpy: return "Memcpy";
  case kEventAtomicLoad: return "AtomicLoad";
  case kEventAtomicStore: return "AtomicStore";
  case kEventAtomicRMW: return "AtomicRMW";
  case kEventAtomicCAS: return "AtomicCAS";
  case kEventAtomicFence: return "AtomicFence";
  case kEventReturn: return "Return";
  case kEventAtExit: return "AtExit";
  case kEventMutexLock: return "MutexLock";
  case kEventMutexUnlock: return "MutexUnlock";
  case kEventIgnoreBegin: return "IgnoreBegin";
  case kEventIgnoreEnd: return "IgnoreEnd";
...
private:
  ...
  GlobalVariable *TsanIgnoreCounter;
  Value *Channel;
  ...
  GlobalVariable *IgnoreFormatString;
#endif
...
void ThreadSanitizer::initialize(Module &M, const TargetLibraryInfo &TLI) {
  ...
  auto *IgnoreCounter =
      M.getOrInsertGlobal("__tsan_ignore_events", IRB.getInt32Ty(), [&] {
        auto *GV = new GlobalVariable(M, IRB.getInt32Ty(), /*isConstant=*/false,
                                      GlobalValue::ExternalLinkage, nullptr,
                                      "__tsan_ignore_events", nullptr,
                                      GlobalVariable::InitialExecTLSModel);
        appendToCompilerUsed(M, GV);
        return GV;
      });
  TsanIgnoreCounter = cast<GlobalVariable>(IgnoreCounter);
  auto *AtomicCounters =
  ...
  //     M.getContext(), "Event: channel=%p idx=%x\n");
  // EventFormatString = new GlobalVariable(
  //     M, EventFormatStrC->getType(), true, GlobalValue::PrivateLinkage,
  //     EventFormatStrC, "event format string");

  Constant *IgnoreFormatStrC = ConstantDataArray::getString(
      M.getContext(), "Ignore check: event=%s counter=%d\n");
  IgnoreFormatString = new GlobalVariable(
      M, IgnoreFormatStrC->getType(), true, GlobalValue::PrivateLinkage,
      IgnoreFormatStrC, "ignore format string");
#endif
...
void ThreadSanitizer::InsertRuntimeIgnores(Function &F) {
  ...
  InsertEventSend(IRB, kEventIgnoreBegin);
#endif
...
#endif
#if MONITOR_CALL_LOGGER
    InsertEventSend(*AtExit, kEventIgnoreEnd);
#endif
  }
}
...
Value* ThreadSanitizer::MakeEvent(IRBuilder<> &IRB, EventType Eid, Value *Addr) {
  ...
  auto *AddrTrunc = IRB.CreateAnd(AddrCast, IRB.getInt64(Mask));
  auto *Event = IRB.CreateOr(Id, AddrTrunc);
  return Event;
}
...
void ThreadSanitizer::InsertEventSend(IRBuilder<> &IRB, EventType Eid,
  ...
  bool CouldIgnore = isIgnoreEligible(Eid);
  Value *IgnoreValue = nullptr;
  Value *ShouldLog = nullptr;
  Instruction *IgnoreThen = nullptr;

  if (CouldIgnore) {
    IgnoreValue =
        IRB.CreateLoad(TsanIgnoreCounter->getValueType(), TsanIgnoreCounter);
    ShouldLog = IRB.CreateICmpEQ(IgnoreValue, IRB.getInt32(0));
    ...
    Value *EventTypePtr = IRB.CreateGlobalStringPtr(getEventTypeName(Eid));
    Value *IgnoreArgs[] = {IgnoreFormatString, EventTypePtr, IgnoreValue};
    IRB.CreateCall(Printf, IgnoreArgs);
#endif

    IgnoreThen =
        SplitBlockAndInsertIfThen(ShouldLog, IRB.GetInsertPoint(), false);
    IRB.SetInsertPoint(IgnoreThen);
  }
...

  {
...
#else
    IRB.CreateStore(NextIndx, TsanChannelIdx);
#endif
    ...
    //                    PointerType::get(Type::getInt8Ty(M->getContext()), 0),
    //                    true));
    // Value *DbgArgs[] = {EventFormatString, Channel, Idx};
    // IRB.CreateCall(Printf, DbgArgs);
#endif
  }
...
bool ThreadSanitizer::sanitizeFunction(Function &F,
  ...
  if ((Res || HasCalls) && ClInstrumentFuncEntryExit) {
    ...
    InsertEventSend(IRB, kEventReturn, ReturnAddress);
    ...
    while (IRBuilder<> *AtExit = EE.Next()) {
      InstrumentationIRBuilder::ensureDebugInfo(*AtExit, F);
      // AtExit->CreateCall(TsanFuncExit, {});
      InsertEventSend(*AtExit, kEventAtExit);
    }
    Res = true;
  }
...
bool ThreadSanitizer::instrumentLoadOrStore(const InstructionInfo &II,
...
#endif
#if MONITOR_CALL_LOGGER
    InsertEventSend(IRB, kEventVptrUpdate, Addr, StoredValue);
#endif
...
#endif
#if MONITOR_CALL_LOGGER
    InsertEventSend(IRB, kEventVptrLoad, Addr);
#endif
  ...
  if (IsWrite)
    ...
    InsertEventSend(IRB, kEventRead, Addr, II.Inst);
  }
#endif
...
#endif
#if MONITOR_CALL_LOGGER
  InsertEventSend(IRB, kEventMemset);
#endif
...
#endif
#if MONITOR_CALL_LOGGER
  InsertEventSend(IRB, kEventMemcpy);
#endif
  }
  return false;
}
...
bool ThreadSanitizer::instrumentAtomic(Instruction *I, const DataLayout &DL) {
  ...
  InsertEventSend(IRB, kEventAtomicFence);
#endif
  }
  return true;
}
