  ...
  for (size_t i = 0; i < kNumberOfAccessSizes; ++i) {
    ...
    TsanUnalignedRead[i] =
        M.getOrInsertFunction(UnalignedReadName, Attr, IRB.getVoidTy(),
                              IRB.getPtrTy(), IRB.getPtrTy(), IRB.getInt32Ty());
...
bool ThreadSanitizer::instrumentLoadOrStore(const InstructionInfo &II,
  ...
  // Get the file name and line number of the instrumented instruction
  unsigned LineNumber = 0;
  StringRef FileName = "no_file_name";
  ...
  IRB.CreateCall(OnAccessFunc, {Addr, IRB.CreateGlobalStringPtr(FileName),
                                IRB.getInt32(LineNumber)});
