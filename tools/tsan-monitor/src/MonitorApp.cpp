...
MonitorApp::~MonitorApp() {
  StopAllReaders();
  scheduler_.Stop();
  SPDLOG_DEBUG("MonitorApp stopped");
}
  ...
  while (!stop_.load(std::memory_order_acquire)) {
    ...
    if (scheduler_.TerminationRequested()) {
      if (!scheduler_.HasPendingEvents() && AllChannelsRegistered()) {
        ...
        SPDLOG_DEBUG("MonitorApp stopping after program end");
        break;
      }
    }
...
void MonitorApp::RefreshChannels() {
  for (const auto& entry : std::filesystem::directory_iterator(directory_)) {
    ...
    SPDLOG_DEBUG("MonitorApp: registered channel {}", tid);
  }
}
...
bool MonitorApp::AllChannelsRegistered() const {
  for (const auto& entry : std::filesystem::directory_iterator(directory_)) {
    ...
    std::lock_guard<std::mutex> lock(threads_mutex_);
    if (threads_.find(tid) == threads_.end()) {
      SPDLOG_DEBUG("MonitorApp: channel {} not yet registered", tid);
      return false;
    }
  }
  SPDLOG_DEBUG("MonitorApp: all channels registered");
  return true;
}
}  // namespace monitor
