...
namespace monitor {
  ...
  std::shared_ptr<Channel> Channel::Open(const std::filesystem::path &path,
    ...
    void *mapping = ::mmap(nullptr, st.st_size, PROT_READ | PROT_WRITE, MAP_SHARED,
                           fd, 0);
    ...
    auto deleter = [fd, mapping, size = static_cast<std::size_t>(st.st_size)](Channel *c) {
      spdlog::debug("Channel {} closed", c->tid_);
      delete c;
      ::munmap(mapping, size);
      ::close(fd);
    };
    ...
    auto channel = std::shared_ptr<Channel>(
      new Channel(path, tid, slots), deleter);
    return channel;
  }

  Channel::Channel(const std::filesystem::path &path,
                   int tid,
                   Slot *slots)
  ...
  bool Channel::TryRead(Event &event) {
    ...
    const std::size_t slot_index = static_cast<std::size_t>(next_index_ & kSlotMask);
    ...
    for (std::size_t i = 0; i < nargs; ++i) {
      ...
      event.args[i] =
          slots_[arg_slot].load(std::memory_order_relaxed);
    }
