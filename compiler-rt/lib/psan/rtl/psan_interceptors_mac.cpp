...
#include "psan_spinlock_defs_mac.h"
...
#define OSATOMIC_INTERCEPTOR(return_t, t, psan_t, f, psan_atomic_f, mo) \
  PSAN_INTERCEPTOR(return_t, f, t x, volatile t *ptr) {                 \
    SCOPED_PSAN_INTERCEPTOR(f, x, ptr);                                 \
    return psan_atomic_f((volatile psan_t *)ptr, x, mo);                \
  }

#define OSATOMIC_INTERCEPTOR_PLUS_X(return_t, t, psan_t, f, psan_atomic_f, mo) \
  PSAN_INTERCEPTOR(return_t, f, t x, volatile t *ptr) {                        \
    SCOPED_PSAN_INTERCEPTOR(f, x, ptr);                                        \
    return psan_atomic_f((volatile psan_t *)ptr, x, mo) + x;                   \
  }

#define OSATOMIC_INTERCEPTOR_PLUS_1(return_t, t, psan_t, f, psan_atomic_f, mo) \
  PSAN_INTERCEPTOR(return_t, f, volatile t *ptr) {                             \
    SCOPED_PSAN_INTERCEPTOR(f, ptr);                                           \
    return psan_atomic_f((volatile psan_t *)ptr, 1, mo) + 1;                   \
  }

#define OSATOMIC_INTERCEPTOR_MINUS_1(return_t, t, psan_t, f, psan_atomic_f, \
                                     mo)                                    \
  PSAN_INTERCEPTOR(return_t, f, volatile t *ptr) {                          \
    SCOPED_PSAN_INTERCEPTOR(f, ptr);                                        \
    return psan_atomic_f((volatile psan_t *)ptr, 1, mo) - 1;                \
...
#define OSATOMIC_INTERCEPTORS_ARITHMETIC(f, psan_atomic_f, m)                  \
  m(int32_t, int32_t, a32, f##32, __psan_atomic32_##psan_atomic_f,             \
    kMacOrderNonBarrier)                                                       \
  m(int32_t, int32_t, a32, f##32##Barrier, __psan_atomic32_##psan_atomic_f,    \
    kMacOrderBarrier)                                                          \
  m(int64_t, int64_t, a64, f##64, __psan_atomic64_##psan_atomic_f,             \
    kMacOrderNonBarrier)                                                       \
  m(int64_t, int64_t, a64, f##64##Barrier, __psan_atomic64_##psan_atomic_f,    \
    kMacOrderBarrier)

#define OSATOMIC_INTERCEPTORS_BITWISE(f, psan_atomic_f, m, m_orig)             \
  m(int32_t, uint32_t, a32, f##32, __psan_atomic32_##psan_atomic_f,            \
    kMacOrderNonBarrier)                                                       \
  m(int32_t, uint32_t, a32, f##32##Barrier, __psan_atomic32_##psan_atomic_f,   \
    kMacOrderBarrier)                                                          \
  m_orig(int32_t, uint32_t, a32, f##32##Orig, __psan_atomic32_##psan_atomic_f, \
    kMacOrderNonBarrier)                                                       \
  m_orig(int32_t, uint32_t, a32, f##32##OrigBarrier,                           \
    __psan_atomic32_##psan_atomic_f, kMacOrderBarrier)
...
#define OSATOMIC_INTERCEPTORS_CAS(f, psan_atomic_f, psan_t, t)              \
  PSAN_INTERCEPTOR(bool, f, t old_value, t new_value, t volatile *ptr) {    \
    SCOPED_PSAN_INTERCEPTOR(f, old_value, new_value, ptr);                  \
    return psan_atomic_f##_compare_exchange_strong(                         \
        (volatile psan_t *)ptr, (psan_t *)&old_value, (psan_t)new_value,    \
        kMacOrderNonBarrier, kMacFailureOrder);                             \
  }                                                                         \
                                                                            \
  PSAN_INTERCEPTOR(bool, f##Barrier, t old_value, t new_value,              \
                   t volatile *ptr) {                                       \
    SCOPED_PSAN_INTERCEPTOR(f##Barrier, old_value, new_value, ptr);         \
    return psan_atomic_f##_compare_exchange_strong(                         \
        (volatile psan_t *)ptr, (psan_t *)&old_value, (psan_t)new_value,    \
        kMacOrderBarrier, kMacFailureOrder);                                \
  }

OSATOMIC_INTERCEPTORS_CAS(OSAtomicCompareAndSwapInt, __psan_atomic32, a32, int)
OSATOMIC_INTERCEPTORS_CAS(OSAtomicCompareAndSwapLong, __psan_atomic64, a64,
                          long_t)
OSATOMIC_INTERCEPTORS_CAS(OSAtomicCompareAndSwapPtr, __psan_atomic64, a64,
                          void *)
OSATOMIC_INTERCEPTORS_CAS(OSAtomicCompareAndSwap32, __psan_atomic32, a32,
                          int32_t)
OSATOMIC_INTERCEPTORS_CAS(OSAtomicCompareAndSwap64, __psan_atomic64, a64,
                          int64_t)

#define OSATOMIC_INTERCEPTOR_BITOP(f, op, clear, mo)             \
  PSAN_INTERCEPTOR(bool, f, uint32_t n, volatile void *ptr) {    \
    SCOPED_PSAN_INTERCEPTOR(f, n, ptr);                          \
...
OSATOMIC_INTERCEPTORS_BITOP(OSAtomicTestAndSet, __psan_atomic8_fetch_or, false)
OSATOMIC_INTERCEPTORS_BITOP(OSAtomicTestAndClear, __psan_atomic8_fetch_and,
                            true)

PSAN_INTERCEPTOR(void, OSAtomicEnqueue, OSQueueHead *list, void *item,
                 size_t offset) {
  SCOPED_PSAN_INTERCEPTOR(OSAtomicEnqueue, list, item, offset);
  __psan_release(item);
  REAL(OSAtomicEnqueue)(list, item, offset);
}

PSAN_INTERCEPTOR(void *, OSAtomicDequeue, OSQueueHead *list, size_t offset) {
  SCOPED_PSAN_INTERCEPTOR(OSAtomicDequeue, list, offset);
  void *item = REAL(OSAtomicDequeue)(list, offset);
  if (item) __psan_acquire(item);
  return item;
}
...
PSAN_INTERCEPTOR(void, OSAtomicFifoEnqueue, OSFifoQueueHead *list, void *item,
                 size_t offset) {
  SCOPED_PSAN_INTERCEPTOR(OSAtomicFifoEnqueue, list, item, offset);
  __psan_release(item);
  REAL(OSAtomicFifoEnqueue)(list, item, offset);
}

PSAN_INTERCEPTOR(void *, OSAtomicFifoDequeue, OSFifoQueueHead *list,
                 size_t offset) {
  SCOPED_PSAN_INTERCEPTOR(OSAtomicFifoDequeue, list, offset);
...
PSAN_INTERCEPTOR(void, OSSpinLockLock, volatile OSSpinLock *lock) {
  CHECK(!cur_thread()->is_dead);
  if (!cur_thread()->is_inited) {
    return REAL(OSSpinLockLock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(OSSpinLockLock, lock);
  REAL(OSSpinLockLock)(lock);
  Acquire(thr, pc, (uptr)lock);
}

PSAN_INTERCEPTOR(bool, OSSpinLockTry, volatile OSSpinLock *lock) {
  CHECK(!cur_thread()->is_dead);
  if (!cur_thread()->is_inited) {
    return REAL(OSSpinLockTry)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(OSSpinLockTry, lock);
...
PSAN_INTERCEPTOR(void, OSSpinLockUnlock, volatile OSSpinLock *lock) {
  CHECK(!cur_thread()->is_dead);
  if (!cur_thread()->is_inited) {
    return REAL(OSSpinLockUnlock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(OSSpinLockUnlock, lock);
  Release(thr, pc, (uptr)lock);
  REAL(OSSpinLockUnlock)(lock);
}

PSAN_INTERCEPTOR(void, os_lock_lock, void *lock) {
  CHECK(!cur_thread()->is_dead);
  if (!cur_thread()->is_inited) {
    return REAL(os_lock_lock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(os_lock_lock, lock);
  REAL(os_lock_lock)(lock);
  Acquire(thr, pc, (uptr)lock);
}

PSAN_INTERCEPTOR(bool, os_lock_trylock, void *lock) {
  CHECK(!cur_thread()->is_dead);
  if (!cur_thread()->is_inited) {
    return REAL(os_lock_trylock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(os_lock_trylock, lock);
...
PSAN_INTERCEPTOR(void, os_lock_unlock, void *lock) {
  CHECK(!cur_thread()->is_dead);
  if (!cur_thread()->is_inited) {
    return REAL(os_lock_unlock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(os_lock_unlock, lock);
  Release(thr, pc, (uptr)lock);
  REAL(os_lock_unlock)(lock);
}

PSAN_INTERCEPTOR(void, os_unfair_lock_lock, os_unfair_lock_t lock) {
  if (!cur_thread()->is_inited || cur_thread()->is_dead) {
    return REAL(os_unfair_lock_lock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(os_unfair_lock_lock, lock);
  REAL(os_unfair_lock_lock)(lock);
  Acquire(thr, pc, (uptr)lock);
}

PSAN_INTERCEPTOR(void, os_unfair_lock_lock_with_options, os_unfair_lock_t lock,
                 u32 options) {
  if (!cur_thread()->is_inited || cur_thread()->is_dead) {
    return REAL(os_unfair_lock_lock_with_options)(lock, options);
  }
  SCOPED_PSAN_INTERCEPTOR(os_unfair_lock_lock_with_options, lock, options);
  REAL(os_unfair_lock_lock_with_options)(lock, options);
  Acquire(thr, pc, (uptr)lock);
}

PSAN_INTERCEPTOR(bool, os_unfair_lock_trylock, os_unfair_lock_t lock) {
  if (!cur_thread()->is_inited || cur_thread()->is_dead) {
    return REAL(os_unfair_lock_trylock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(os_unfair_lock_trylock, lock);
...
PSAN_INTERCEPTOR(void, os_unfair_lock_unlock, os_unfair_lock_t lock) {
  if (!cur_thread()->is_inited || cur_thread()->is_dead) {
    return REAL(os_unfair_lock_unlock)(lock);
  }
  SCOPED_PSAN_INTERCEPTOR(os_unfair_lock_unlock, lock);
  Release(thr, pc, (uptr)lock);
...
PSAN_INTERCEPTOR(void, xpc_connection_set_event_handler,
                 xpc_connection_t connection, xpc_handler_t handler) {
  SCOPED_PSAN_INTERCEPTOR(xpc_connection_set_event_handler, connection,
                          handler);
...
PSAN_INTERCEPTOR(void, xpc_connection_send_barrier, xpc_connection_t connection,
                 dispatch_block_t barrier) {
  SCOPED_PSAN_INTERCEPTOR(xpc_connection_send_barrier, connection, barrier);
  Release(thr, pc, (uptr)connection);
...
PSAN_INTERCEPTOR(void, xpc_connection_send_message_with_reply,
                 xpc_connection_t connection, xpc_object_t message,
                 dispatch_queue_t replyq, xpc_handler_t handler) {
  SCOPED_PSAN_INTERCEPTOR(xpc_connection_send_message_with_reply, connection,
...
PSAN_INTERCEPTOR(void, xpc_connection_cancel, xpc_connection_t connection) {
  SCOPED_PSAN_INTERCEPTOR(xpc_connection_cancel, connection);
  Release(thr, pc, (uptr)connection);
...
// Returns an address which can be used to inform PSan about synchronization
// points (MutexLock/Unlock). The PSan infrastructure expects this to be a valid
...
PSAN_INTERCEPTOR(int, objc_sync_enter, id obj) {
  SCOPED_PSAN_INTERCEPTOR(objc_sync_enter, obj);
...
PSAN_INTERCEPTOR(int, objc_sync_exit, id obj) {
  SCOPED_PSAN_INTERCEPTOR(objc_sync_exit, obj);
...
PSAN_INTERCEPTOR(int, swapcontext, ucontext_t *oucp, const ucontext_t *ucp) {
  {
...
// libc++.dylib, which are PSan-invisible, and this leads to false positives in
...
STDCXX_INTERCEPTOR(void, _ZNSt3__119__shared_weak_count16__release_sharedEv,
  ...
  SCOPED_PSAN_INTERCEPTOR(_ZNSt3__119__shared_weak_count16__release_sharedEv,
                          o);
  if (__psan_atomic64_fetch_add(&o->shared_owners, -1, mo_release) == 0) {
    Acquire(thr, pc, (uptr)&o->shared_owners);
    o->on_zero_shared();
    if (__psan_atomic64_fetch_add(&o->shared_weak_owners, -1, mo_release) ==
        0) {
...
STDCXX_INTERCEPTOR(bool, _ZNSt3__114__shared_count16__release_sharedEv,
  ...
  SCOPED_PSAN_INTERCEPTOR(_ZNSt3__114__shared_count16__release_sharedEv, o);
  if (__psan_atomic64_fetch_add(&o->shared_owners, -1, mo_release) == 0) {
    Acquire(thr, pc, (uptr)&o->shared_owners);
    o->on_zero_shared();
    return true;
...
// PSan-invisible. To avoid false positives, this interceptor wraps the callback
