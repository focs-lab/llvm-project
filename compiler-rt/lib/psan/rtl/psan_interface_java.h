...
// The interface is intended to be used from within a JVM and notify PSan
...
// C++ interfaces: __psan_readN/writeN and __psan_func_enter/exit.
//
// For volatile memory accesses and atomic operations JVM is intended to use
// standard atomics API: __psan_atomicN_load/store/etc.
//
// For usage examples see lit_tests/java_*.cpp
//===----------------------------------------------------------------------===//
#ifndef PSAN_INTERFACE_JAVA_H
#define PSAN_INTERFACE_JAVA_H
...
void __psan_java_init(jptr heap_begin, jptr heap_size) INTERFACE_ATTRIBUTE;
// Must be called when the application exits.
// Not necessary the last callback (concurrently running threads are OK).
// Returns exit status or 0 if psan does not want to override it.
int  __psan_java_fini() INTERFACE_ATTRIBUTE;
...
void __psan_java_alloc(jptr ptr, jptr size) INTERFACE_ATTRIBUTE;
// Callback for memory free.
// Can be aggregated for several objects (preferably).
void __psan_java_free(jptr ptr, jptr size) INTERFACE_ATTRIBUTE;
// Callback for memory move by GC.
// Can be aggregated for several objects (preferably).
// The ranges can overlap.
void __psan_java_move(jptr src, jptr dst, jptr size) INTERFACE_ATTRIBUTE;
...
void __psan_java_finalize() INTERFACE_ATTRIBUTE;
...
jptr __psan_java_find(jptr *from_ptr, jptr to) INTERFACE_ATTRIBUTE;

// Mutex lock.
// Addr is any unique address associated with the mutex.
// Can be called on recursive reentry.
void __psan_java_mutex_lock(jptr addr) INTERFACE_ATTRIBUTE;
// Mutex unlock.
void __psan_java_mutex_unlock(jptr addr) INTERFACE_ATTRIBUTE;
// Mutex read lock.
void __psan_java_mutex_read_lock(jptr addr) INTERFACE_ATTRIBUTE;
// Mutex read unlock.
void __psan_java_mutex_read_unlock(jptr addr) INTERFACE_ATTRIBUTE;
// Recursive mutex lock, intended for handling of Object.wait().
// The 'rec' value must be obtained from the previous
// __psan_java_mutex_unlock_rec().
void __psan_java_mutex_lock_rec(jptr addr, int rec) INTERFACE_ATTRIBUTE;
...
// It must be passed back to __psan_java_mutex_lock_rec() to restore
// the same recursion level.
int __psan_java_mutex_unlock_rec(jptr addr) INTERFACE_ATTRIBUTE;
...
void __psan_java_acquire(jptr addr) INTERFACE_ATTRIBUTE;
void __psan_java_release(jptr addr) INTERFACE_ATTRIBUTE;
void __psan_java_release_store(jptr addr) INTERFACE_ATTRIBUTE;
...
#endif  // #ifndef PSAN_INTERFACE_JAVA_H
