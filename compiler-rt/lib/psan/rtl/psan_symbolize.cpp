...
bool __psan_symbolize_external(uptr pc, char *func_buf, uptr func_siz,
                               char *file_buf, uptr file_siz, int *line,
                               int *col) {
  return false;
}

// New API: call __psan_symbolize_external_ex only when it exists.
// Once old clients are gone, provide dummy implementation.
SANITIZER_WEAK_DEFAULT_IMPL
void __psan_symbolize_external_ex(uptr pc,
...
SymbolizedStack *SymbolizeCode(uptr addr) {
  ...
  if (addr & kExternalPCBit) {
    ...
    __psan_symbolize_external_ex(addr, AddFrame, &ssb);
    if (ssb.head)
      return ssb.head;
    ...
    if (__psan_symbolize_external(addr, func_buf, sizeof(func_buf), file_buf,
