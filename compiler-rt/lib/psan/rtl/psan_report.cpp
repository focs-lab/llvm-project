...
static const SymbolizedStack *SkipPsanInternalFrames(SymbolizedStack *frames) {
...
void PrintReport(const ReportDesc *rep) {
  ...
  if (rep->typ == ReportTypeDeadlock) {
    ...
    for (uptr i = 0; i < rep->mutexes.Size(); i++) {
      ...
      if (flags()->second_deadlock_stack) {
        ...
        if (i == 0)
          Printf("    Hint: use PSAN_OPTIONS=second_deadlock_stack=1 "
  ...
  if (ReportStack *stack = ChooseSummaryStack(rep)) {
    if (const SymbolizedStack *frame = SkipPsanInternalFrames(stack->frames))
