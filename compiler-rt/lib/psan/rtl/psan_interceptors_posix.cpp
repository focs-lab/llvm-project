...
// The sole reason psan wraps atexit callbacks is to establish synchronization
...
#define PSAN_INTERCEPT(func) INTERCEPT_FUNCTION(func)
#if SANITIZER_FREEBSD || SANITIZER_NETBSD
#  define PSAN_INTERCEPT_VER(func, ver) INTERCEPT_FUNCTION(func)
#else
#  define PSAN_INTERCEPT_VER(func, ver) INTERCEPT_FUNCTION_VER(func, ver)
#endif
#if SANITIZER_FREEBSD
#  define PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(func) \
    INTERCEPT_FUNCTION(_pthread_##func)
#else
#  define PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(func)
#endif
#if SANITIZER_NETBSD
#  define PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(func) \
    INTERCEPT_FUNCTION(__libc_##func)
#  define PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS_THR(func) \
...
PSAN_INTERCEPTOR(unsigned, sleep, unsigned sec) {
  SCOPED_PSAN_INTERCEPTOR(sleep, sec);
  unsigned res = BLOCK_REAL(sleep)(sec);
  AfterSleep(thr, pc);
  return res;
}

PSAN_INTERCEPTOR(int, usleep, long_t usec) {
  SCOPED_PSAN_INTERCEPTOR(usleep, usec);
  int res = BLOCK_REAL(usleep)(usec);
  AfterSleep(thr, pc);
  return res;
}

PSAN_INTERCEPTOR(int, nanosleep, void *req, void *rem) {
  SCOPED_PSAN_INTERCEPTOR(nanosleep, req, rem);
  int res = BLOCK_REAL(nanosleep)(req, rem);
  AfterSleep(thr, pc);
  return res;
}

PSAN_INTERCEPTOR(int, pause, int fake) {
  SCOPED_PSAN_INTERCEPTOR(pause, fake);
...
PSAN_INTERCEPTOR(int, atexit, void (*f)()) {
  if (in_symbolizer())
    return 0;
...
PSAN_INTERCEPTOR(int, __cxa_atexit, void (*f)(void *a), void *arg, void *dso) {
  if (in_symbolizer())
    return 0;
  SCOPED_PSAN_INTERCEPTOR(__cxa_atexit, f, arg, dso);
...
PSAN_INTERCEPTOR(int, on_exit, void(*f)(int, void*), void *arg) {
  if (in_symbolizer())
    return 0;
  SCOPED_PSAN_INTERCEPTOR(on_exit, f, arg);
  auto *ctx = New<AtExitCtx>();
  ctx->f = (void(*)())f;
  ctx->arg = arg;
...
#define PSAN_MAYBE_INTERCEPT_ON_EXIT PSAN_INTERCEPT(on_exit)
#else
#define PSAN_MAYBE_INTERCEPT_ON_EXIT
#endif
...
static void LongJmp(ThreadState *thr, uptr *env) {
  ...
  Printf("PredictiveSanitizer: can't find longjmp buf\n");
  CHECK(0);
}

// FIXME: put everything below into a common extern "C" block?
extern "C" void __psan_setjmp(uptr sp) { SetJmp(cur_thread_init(), sp); }

#if SANITIZER_APPLE
PSAN_INTERCEPTOR(int, setjmp, void *env);
PSAN_INTERCEPTOR(int, _setjmp, void *env);
PSAN_INTERCEPTOR(int, sigsetjmp, void *env);
...
static void InitializeSetjmpInterceptors() {
  // We can not use PSAN_INTERCEPT to get setjmp addr, because it does &setjmp and
...
PSAN_INTERCEPTOR(void, longjmp_symname, uptr *env, int val) {
...
PSAN_INTERCEPTOR(void, siglongjmp_symname, uptr *env, int val) {
  {
...
#if SANITIZER_NETBSD
PSAN_INTERCEPTOR(void, _longjmp, uptr *env, int val) {
  {
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(void*, malloc, uptr size) {
  if (in_symbolizer())
...
PSAN_INTERCEPTOR(void*, __libc_memalign, uptr align, uptr sz) {
  SCOPED_INTERCEPTOR_RAW(__libc_memalign, align, sz);
  return user_memalign(thr, pc, align, sz);
}

PSAN_INTERCEPTOR(void*, calloc, uptr size, uptr n) {
  if (in_symbolizer())
...
PSAN_INTERCEPTOR(void*, realloc, void *p, uptr size) {
  if (in_symbolizer())
...
PSAN_INTERCEPTOR(void*, reallocarray, void *p, uptr size, uptr n) {
  if (in_symbolizer())
...
PSAN_INTERCEPTOR(void, free, void *p) {
  if (p == 0)
    return;
  if (in_symbolizer())
...
PSAN_INTERCEPTOR(void, cfree, void *p) {
  if (p == 0)
    return;
  if (in_symbolizer())
    return InternalFree(p);
...
PSAN_INTERCEPTOR(uptr, malloc_usable_size, void *p) {
  SCOPED_INTERCEPTOR_RAW(malloc_usable_size, p);
  return user_alloc_usable_size(p);
}
#endif

PSAN_INTERCEPTOR(char *, strcpy, char *dst, const char *src) {
  SCOPED_PSAN_INTERCEPTOR(strcpy, dst, src);
...
PSAN_INTERCEPTOR(char*, strncpy, char *dst, char *src, uptr n) {
  SCOPED_PSAN_INTERCEPTOR(strncpy, dst, src, n);
...
PSAN_INTERCEPTOR(char*, strdup, const char *str) {
  SCOPED_PSAN_INTERCEPTOR(strdup, str);
...
static void *mmap_interceptor(ThreadState *thr, uptr pc, Mmap real_mmap,
  ...
  if (res != MAP_FAILED) {
    if (!IsAppMem((uptr)res) || !IsAppMem((uptr)res + sz - 1)) {
      Report("PredictiveSanitizer: mmap at bad address: addr=%p size=%p res=%p\n",
             addr, (void*)sz, res);
      Die();
    }
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(void*, memalign, uptr align, uptr sz) {
  SCOPED_INTERCEPTOR_RAW(memalign, align, sz);
  return user_memalign(thr, pc, align, sz);
}
#define PSAN_MAYBE_INTERCEPT_MEMALIGN PSAN_INTERCEPT(memalign)
#else
#define PSAN_MAYBE_INTERCEPT_MEMALIGN
#endif

#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(void*, aligned_alloc, uptr align, uptr sz) {
  if (in_symbolizer())
...
PSAN_INTERCEPTOR(void*, valloc, uptr sz) {
  if (in_symbolizer())
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(void*, pvalloc, uptr sz) {
  if (in_symbolizer()) {
...
#define PSAN_MAYBE_INTERCEPT_PVALLOC PSAN_INTERCEPT(pvalloc)
#else
#define PSAN_MAYBE_INTERCEPT_PVALLOC
#endif

#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, posix_memalign, void **memptr, uptr align, uptr sz) {
  if (in_symbolizer()) {
...
// interceptor.
#if SANITIZER_APPLE
#define STDCXX_INTERCEPTOR PSAN_INTERCEPTOR
#else
...
static void thread_finalize(void *v) {
  ...
  if (iter > 1) {
    if (pthread_setspecific(interceptor_ctx()->finalize_key,
        (void*)(iter - 1))) {
      Printf("PredictiveSanitizer: failed to set thread key\n");
      Die();
    }
    return;
  }
...
extern "C" void *__psan_thread_start_func(void *arg) {
...
PSAN_INTERCEPTOR(int, pthread_create,
  ...
  if (ctx->after_multithreaded_fork) {
    if (flags()->die_after_fork) {
      Report("PredictiveSanitizer: starting new threads after multi-threaded "
          "fork is not supported. Dying (set die_after_fork=0 to override)\n");
      Die();
    } else {
      VPrintf(1,
              "PredictiveSanitizer: starting new threads after multi-threaded "
  ...
  int res = -1;
  {
    ...
    ScopedIgnoreInterceptors ignore;
    ThreadIgnoreBegin(thr, pc);
    res = REAL(pthread_create)(th, attr, __psan_thread_start_func, &p);
    ThreadIgnoreEnd(thr);
  }
  if (res == 0) {
...
PSAN_INTERCEPTOR(int, pthread_join, void *th, void **ret) {
...
PSAN_INTERCEPTOR(int, pthread_detach, void *th) {
...
PSAN_INTERCEPTOR(void, pthread_exit, void *retval) {
  {
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, pthread_tryjoin_np, void *th, void **ret) {
...
PSAN_INTERCEPTOR(int, pthread_timedjoin_np, void *th, void **ret,
...
void CondMutexUnlockCtx<Fn>::Unlock() const {
  ...
  // psan code. Also ScopedInterceptor and BlockingCall destructors won't run
...
INTERCEPTOR(int, pthread_cond_init, void *c, void *a) {
  ...
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_init, cond, a);
...
INTERCEPTOR(int, pthread_cond_wait, void *c, void *m) {
  ...
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_wait, cond, m);
  return cond_wait(
...
INTERCEPTOR(int, pthread_cond_timedwait, void *c, void *m, void *abstime) {
  void *cond = init_cond(c);
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_timedwait, cond, m, abstime);
  return cond_wait(
      thr, pc, &si,
...
INTERCEPTOR(int, pthread_cond_clockwait, void *c, void *m,
  ...
  void *cond = init_cond(c);
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_clockwait, cond, m, clock, abstime);
  return cond_wait(
      thr, pc, &si,
      [=]() { return REAL(pthread_cond_clockwait)(cond, m, clock, abstime); },
      cond, m);
}
#define PSAN_MAYBE_PTHREAD_COND_CLOCKWAIT PSAN_INTERCEPT(pthread_cond_clockwait)
#else
#define PSAN_MAYBE_PTHREAD_COND_CLOCKWAIT
#endif
...
INTERCEPTOR(int, pthread_cond_timedwait_relative_np, void *c, void *m,
            void *reltime) {
  void *cond = init_cond(c);
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_timedwait_relative_np, cond, m, reltime);
  return cond_wait(
      thr, pc, &si,
      [=]() {
...
INTERCEPTOR(int, pthread_cond_signal, void *c) {
  void *cond = init_cond(c);
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_signal, cond);
  MemoryAccessRange(thr, pc, (uptr)c, sizeof(uptr), false);
...
INTERCEPTOR(int, pthread_cond_broadcast, void *c) {
  void *cond = init_cond(c);
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_broadcast, cond);
  MemoryAccessRange(thr, pc, (uptr)c, sizeof(uptr), false);
...
INTERCEPTOR(int, pthread_cond_destroy, void *c) {
  void *cond = init_cond(c);
  SCOPED_PSAN_INTERCEPTOR(pthread_cond_destroy, cond);
  MemoryAccessRange(thr, pc, (uptr)c, sizeof(uptr), true);
...
PSAN_INTERCEPTOR(int, pthread_mutex_init, void *m, void *a) {
  SCOPED_PSAN_INTERCEPTOR(pthread_mutex_init, m, a);
...
PSAN_INTERCEPTOR(int, pthread_mutex_destroy, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_mutex_destroy, m);
...
PSAN_INTERCEPTOR(int, pthread_mutex_lock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_mutex_lock, m);
...
PSAN_INTERCEPTOR(int, pthread_mutex_trylock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_mutex_trylock, m);
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, pthread_mutex_timedlock, void *m, void *abstime) {
  SCOPED_PSAN_INTERCEPTOR(pthread_mutex_timedlock, m, abstime);
...
PSAN_INTERCEPTOR(int, pthread_mutex_unlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_mutex_unlock, m);
  MutexUnlock(thr, pc, (uptr)m);
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, pthread_mutex_clocklock, void *m,
                 __sanitizer_clockid_t clock, void *abstime) {
  SCOPED_PSAN_INTERCEPTOR(pthread_mutex_clocklock, m, clock, abstime);
  MutexPreLock(thr, pc, (uptr)m);
...
PSAN_INTERCEPTOR(int, __pthread_mutex_lock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(__pthread_mutex_lock, m);
  MutexPreLock(thr, pc, (uptr)m);
...
PSAN_INTERCEPTOR(int, __pthread_mutex_unlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(__pthread_mutex_unlock, m);
  MutexUnlock(thr, pc, (uptr)m);
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, pthread_spin_init, void *m, int pshared) {
  SCOPED_PSAN_INTERCEPTOR(pthread_spin_init, m, pshared);
...
PSAN_INTERCEPTOR(int, pthread_spin_destroy, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_spin_destroy, m);
...
PSAN_INTERCEPTOR(int, pthread_spin_lock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_spin_lock, m);
  MutexPreLock(thr, pc, (uptr)m);
...
PSAN_INTERCEPTOR(int, pthread_spin_trylock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_spin_trylock, m);
...
PSAN_INTERCEPTOR(int, pthread_spin_unlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_spin_unlock, m);
  MutexUnlock(thr, pc, (uptr)m);
...
PSAN_INTERCEPTOR(int, pthread_rwlock_init, void *m, void *a) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_init, m, a);
...
PSAN_INTERCEPTOR(int, pthread_rwlock_destroy, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_destroy, m);
...
PSAN_INTERCEPTOR(int, pthread_rwlock_rdlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_rdlock, m);
...
PSAN_INTERCEPTOR(int, pthread_rwlock_tryrdlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_tryrdlock, m);
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, pthread_rwlock_timedrdlock, void *m, void *abstime) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_timedrdlock, m, abstime);
...
PSAN_INTERCEPTOR(int, pthread_rwlock_wrlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_wrlock, m);
  MutexPreLock(thr, pc, (uptr)m);
...
PSAN_INTERCEPTOR(int, pthread_rwlock_trywrlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_trywrlock, m);
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, pthread_rwlock_timedwrlock, void *m, void *abstime) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_timedwrlock, m, abstime);
...
PSAN_INTERCEPTOR(int, pthread_rwlock_unlock, void *m) {
  SCOPED_PSAN_INTERCEPTOR(pthread_rwlock_unlock, m);
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, pthread_barrier_init, void *b, void *a, unsigned count) {
  SCOPED_PSAN_INTERCEPTOR(pthread_barrier_init, b, a, count);
  MemoryAccess(thr, pc, (uptr)b, 1, kAccessWrite);
  int res = REAL(pthread_barrier_init)(b, a, count);
  return res;
}

PSAN_INTERCEPTOR(int, pthread_barrier_destroy, void *b) {
  SCOPED_PSAN_INTERCEPTOR(pthread_barrier_destroy, b);
  MemoryAccess(thr, pc, (uptr)b, 1, kAccessWrite);
  int res = REAL(pthread_barrier_destroy)(b);
  return res;
}

PSAN_INTERCEPTOR(int, pthread_barrier_wait, void *b) {
  SCOPED_PSAN_INTERCEPTOR(pthread_barrier_wait, b);
...
PSAN_INTERCEPTOR(int, pthread_once, void *o, void (*f)()) {
...
#if SANITIZER_GLIBC
PSAN_INTERCEPTOR(int, __fxstat, int version, int fd, void *buf) {
  SCOPED_PSAN_INTERCEPTOR(__fxstat, version, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(__fxstat)(version, fd, buf);
}

PSAN_INTERCEPTOR(int, __fxstat64, int version, int fd, void *buf) {
  SCOPED_PSAN_INTERCEPTOR(__fxstat64, version, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(__fxstat64)(version, fd, buf);
}
#define PSAN_MAYBE_INTERCEPT___FXSTAT PSAN_INTERCEPT(__fxstat); PSAN_INTERCEPT(__fxstat64)
#else
#define PSAN_MAYBE_INTERCEPT___FXSTAT
#endif

#if !SANITIZER_GLIBC || __GLIBC_PREREQ(2, 33)
PSAN_INTERCEPTOR(int, fstat, int fd, void *buf) {
  SCOPED_PSAN_INTERCEPTOR(fstat, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(fstat)(fd, buf);
}
#  define PSAN_MAYBE_INTERCEPT_FSTAT PSAN_INTERCEPT(fstat)
#else
#  define PSAN_MAYBE_INTERCEPT_FSTAT
#endif

#if __GLIBC_PREREQ(2, 33)
PSAN_INTERCEPTOR(int, fstat64, int fd, void *buf) {
  SCOPED_PSAN_INTERCEPTOR(fstat64, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(fstat64)(fd, buf);
}
#  define PSAN_MAYBE_INTERCEPT_FSTAT64 PSAN_INTERCEPT(fstat64)
#else
#  define PSAN_MAYBE_INTERCEPT_FSTAT64
#endif

PSAN_INTERCEPTOR(int, open, const char *name, int oflag, ...) {
  va_list ap;
  va_start(ap, oflag);
  mode_t mode = va_arg(ap, int);
  va_end(ap);
  SCOPED_PSAN_INTERCEPTOR(open, name, oflag, mode);
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, open64, const char *name, int oflag, ...) {
  va_list ap;
  va_start(ap, oflag);
  mode_t mode = va_arg(ap, int);
  va_end(ap);
  SCOPED_PSAN_INTERCEPTOR(open64, name, oflag, mode);
  READ_STRING(thr, pc, name, 0);
...
#define PSAN_MAYBE_INTERCEPT_OPEN64 PSAN_INTERCEPT(open64)
#else
#define PSAN_MAYBE_INTERCEPT_OPEN64
#endif

PSAN_INTERCEPTOR(int, creat, const char *name, int mode) {
  SCOPED_PSAN_INTERCEPTOR(creat, name, mode);
  READ_STRING(thr, pc, name, 0);
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, creat64, const char *name, int mode) {
  SCOPED_PSAN_INTERCEPTOR(creat64, name, mode);
  READ_STRING(thr, pc, name, 0);
...
#define PSAN_MAYBE_INTERCEPT_CREAT64 PSAN_INTERCEPT(creat64)
#else
#define PSAN_MAYBE_INTERCEPT_CREAT64
#endif

PSAN_INTERCEPTOR(int, dup, int oldfd) {
  SCOPED_PSAN_INTERCEPTOR(dup, oldfd);
...
PSAN_INTERCEPTOR(int, dup2, int oldfd, int newfd) {
  SCOPED_PSAN_INTERCEPTOR(dup2, oldfd, newfd);
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, dup3, int oldfd, int newfd, int flags) {
  SCOPED_PSAN_INTERCEPTOR(dup3, oldfd, newfd, flags);
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, eventfd, unsigned initval, int flags) {
  SCOPED_PSAN_INTERCEPTOR(eventfd, initval, flags);
  int fd = REAL(eventfd)(initval, flags);
  if (fd >= 0)
    FdEventCreate(thr, pc, fd);
  return fd;
}
#define PSAN_MAYBE_INTERCEPT_EVENTFD PSAN_INTERCEPT(eventfd)
#else
#define PSAN_MAYBE_INTERCEPT_EVENTFD
#endif

#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, signalfd, int fd, void *mask, int flags) {
...
#define PSAN_MAYBE_INTERCEPT_SIGNALFD PSAN_INTERCEPT(signalfd)
#else
#define PSAN_MAYBE_INTERCEPT_SIGNALFD
#endif

#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, inotify_init, int fake) {
  SCOPED_PSAN_INTERCEPTOR(inotify_init, fake);
  int fd = REAL(inotify_init)(fake);
  if (fd >= 0)
    FdInotifyCreate(thr, pc, fd);
  return fd;
}
#define PSAN_MAYBE_INTERCEPT_INOTIFY_INIT PSAN_INTERCEPT(inotify_init)
#else
#define PSAN_MAYBE_INTERCEPT_INOTIFY_INIT
#endif

#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, inotify_init1, int flags) {
  SCOPED_PSAN_INTERCEPTOR(inotify_init1, flags);
  int fd = REAL(inotify_init1)(flags);
  if (fd >= 0)
    FdInotifyCreate(thr, pc, fd);
  return fd;
}
#define PSAN_MAYBE_INTERCEPT_INOTIFY_INIT1 PSAN_INTERCEPT(inotify_init1)
#else
#define PSAN_MAYBE_INTERCEPT_INOTIFY_INIT1
#endif

PSAN_INTERCEPTOR(int, socket, int domain, int type, int protocol) {
  SCOPED_PSAN_INTERCEPTOR(socket, domain, type, protocol);
...
PSAN_INTERCEPTOR(int, socketpair, int domain, int type, int protocol, int *fd) {
  SCOPED_PSAN_INTERCEPTOR(socketpair, domain, type, protocol, fd);
...
PSAN_INTERCEPTOR(int, connect, int fd, void *addr, unsigned addrlen) {
  SCOPED_PSAN_INTERCEPTOR(connect, fd, addr, addrlen);
...
PSAN_INTERCEPTOR(int, bind, int fd, void *addr, unsigned addrlen) {
  SCOPED_PSAN_INTERCEPTOR(bind, fd, addr, addrlen);
...
PSAN_INTERCEPTOR(int, listen, int fd, int backlog) {
  SCOPED_PSAN_INTERCEPTOR(listen, fd, backlog);
...
PSAN_INTERCEPTOR(int, close, int fd) {
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, __close, int fd) {
  SCOPED_INTERCEPTOR_RAW(__close, fd);
  FdClose(thr, pc, fd);
  return REAL(__close)(fd);
}
#define PSAN_MAYBE_INTERCEPT___CLOSE PSAN_INTERCEPT(__close)
#else
#define PSAN_MAYBE_INTERCEPT___CLOSE
#endif

// glibc guts
#if SANITIZER_LINUX && !SANITIZER_ANDROID
PSAN_INTERCEPTOR(void, __res_iclose, void *state, bool free_addr) {
...
#define PSAN_MAYBE_INTERCEPT___RES_ICLOSE PSAN_INTERCEPT(__res_iclose)
#else
#define PSAN_MAYBE_INTERCEPT___RES_ICLOSE
#endif

PSAN_INTERCEPTOR(int, pipe, int *pipefd) {
  SCOPED_PSAN_INTERCEPTOR(pipe, pipefd);
...
#if !SANITIZER_APPLE
PSAN_INTERCEPTOR(int, pipe2, int *pipefd, int flags) {
  SCOPED_PSAN_INTERCEPTOR(pipe2, pipefd, flags);
...
PSAN_INTERCEPTOR(int, unlink, char *path) {
  SCOPED_PSAN_INTERCEPTOR(unlink, path);
  Release(thr, pc, File2addr(path));
  int res = REAL(unlink)(path);
  return res;
}

PSAN_INTERCEPTOR(void*, tmpfile, int fake) {
  SCOPED_PSAN_INTERCEPTOR(tmpfile, fake);
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(void*, tmpfile64, int fake) {
  SCOPED_PSAN_INTERCEPTOR(tmpfile64, fake);
...
#define PSAN_MAYBE_INTERCEPT_TMPFILE64 PSAN_INTERCEPT(tmpfile64)
#else
#define PSAN_MAYBE_INTERCEPT_TMPFILE64
#endif
...
PSAN_INTERCEPTOR(void, abort, int fake) {
  SCOPED_PSAN_INTERCEPTOR(abort, fake);
  FlushStreams();
  REAL(abort)(fake);
}

PSAN_INTERCEPTOR(int, rmdir, char *path) {
  SCOPED_PSAN_INTERCEPTOR(rmdir, path);
  Release(thr, pc, Dir2addr(path));
  int res = REAL(rmdir)(path);
  return res;
}

PSAN_INTERCEPTOR(int, closedir, void *dirp) {
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, epoll_create, int size) {
  SCOPED_PSAN_INTERCEPTOR(epoll_create, size);
...
PSAN_INTERCEPTOR(int, epoll_create1, int flags) {
  SCOPED_PSAN_INTERCEPTOR(epoll_create1, flags);
...
PSAN_INTERCEPTOR(int, epoll_ctl, int epfd, int op, int fd, void *ev) {
  SCOPED_PSAN_INTERCEPTOR(epoll_ctl, epfd, op, fd, ev);
  if (epfd >= 0)
...
PSAN_INTERCEPTOR(int, epoll_wait, int epfd, void *ev, int cnt, int timeout) {
  SCOPED_PSAN_INTERCEPTOR(epoll_wait, epfd, ev, cnt, timeout);
  if (epfd >= 0)
    FdAccess(thr, pc, epfd);
...
PSAN_INTERCEPTOR(int, epoll_pwait, int epfd, void *ev, int cnt, int timeout,
                 void *sigmask) {
  SCOPED_PSAN_INTERCEPTOR(epoll_pwait, epfd, ev, cnt, timeout, sigmask);
  if (epfd >= 0)
    FdAccess(thr, pc, epfd);
...
PSAN_INTERCEPTOR(int, epoll_pwait2, int epfd, void *ev, int cnt, void *timeout,
                 void *sigmask) {
...
#  define PSAN_MAYBE_INTERCEPT_EPOLL \
    PSAN_INTERCEPT(epoll_create);    \
    PSAN_INTERCEPT(epoll_create1);   \
    PSAN_INTERCEPT(epoll_ctl);       \
    PSAN_INTERCEPT(epoll_wait);      \
    PSAN_INTERCEPT(epoll_pwait);     \
    PSAN_INTERCEPT(epoll_pwait2)
#else
#define PSAN_MAYBE_INTERCEPT_EPOLL
#endif
...
PSAN_INTERCEPTOR(int, sigsuspend, const __sanitizer_sigset_t *mask) {
  SCOPED_PSAN_INTERCEPTOR(sigsuspend, mask);
  return REAL(sigsuspend)(mask);
}

PSAN_INTERCEPTOR(int, sigblock, int mask) {
  SCOPED_PSAN_INTERCEPTOR(sigblock, mask);
  return REAL(sigblock)(mask);
}

PSAN_INTERCEPTOR(int, sigsetmask, int mask) {
  SCOPED_PSAN_INTERCEPTOR(sigsetmask, mask);
  return REAL(sigsetmask)(mask);
}

PSAN_INTERCEPTOR(int, pthread_sigmask, int how, const __sanitizer_sigset_t *set,
    __sanitizer_sigset_t *oldset) {
  SCOPED_PSAN_INTERCEPTOR(pthread_sigmask, how, set, oldset);
...
static void CallUserSignalHandler(ThreadState *thr, bool sync, bool acquire,
  ...
  // (bug in symbolizer or in psan). But we want to reset
  ...
  // psan reports false positive in such case.
...
void sighandler(int sig, __sanitizer_siginfo *info, void *ctx) {
  ...
  if (sig < 0 || sig >= kSigCount) {
    VPrintf(1, "PredictiveSanitizer: ignoring signal %d\n", sig);
    return;
  }
...
PSAN_INTERCEPTOR(int, raise, int sig) {
  SCOPED_PSAN_INTERCEPTOR(raise, sig);
  ThreadSignalContext *sctx = SigCtx(thr);
  CHECK_NE(sctx, 0);
...
PSAN_INTERCEPTOR(int, kill, int pid, int sig) {
  SCOPED_PSAN_INTERCEPTOR(kill, pid, sig);
  ThreadSignalContext *sctx = SigCtx(thr);
  CHECK_NE(sctx, 0);
  int prev = sctx->int_signal_send;
...
PSAN_INTERCEPTOR(int, pthread_kill, void *tid, int sig) {
  SCOPED_PSAN_INTERCEPTOR(pthread_kill, tid, sig);
  ThreadSignalContext *sctx = SigCtx(thr);
  CHECK_NE(sctx, 0);
  int prev = sctx->int_signal_send;
...
PSAN_INTERCEPTOR(int, gettimeofday, void *tv, void *tz) {
  SCOPED_PSAN_INTERCEPTOR(gettimeofday, tv, tz);
  // It's intercepted merely to process pending signals.
  return REAL(gettimeofday)(tv, tz);
}

PSAN_INTERCEPTOR(int, getaddrinfo, void *node, void *service,
    void *hints, void *rv) {
  SCOPED_PSAN_INTERCEPTOR(getaddrinfo, node, service, hints, rv);
...
PSAN_INTERCEPTOR(int, fork, int fake) {
  if (in_symbolizer())
...
#if !SANITIZER_IOS
PSAN_INTERCEPTOR(int, vfork, int fake) {
  // Some programs (e.g. openjdk) call close for all file descriptors
  // in the child process. Under psan it leads to false positives, because
...
#if SANITIZER_LINUX
PSAN_INTERCEPTOR(int, clone, int (*fn)(void *), void *stack, int flags,
...
PSAN_INTERCEPTOR(int, dl_iterate_phdr, dl_iterate_phdr_cb_t cb, void *data) {
  SCOPED_PSAN_INTERCEPTOR(dl_iterate_phdr, cb, data);
...
#define COMMON_INTERCEPTOR_ENTER_NOIGNORE(ctx, func, ...) \
  ...
  PsanInterceptorContext _ctx = {thr, pc};                \
  ctx = (void *)&_ctx;                                    \
  (void)ctx;
...
#define COMMON_INTERCEPTOR_ACQUIRE(ctx, u) \
  Acquire(((PsanInterceptorContext *) ctx)->thr, pc, u)

#define COMMON_INTERCEPTOR_RELEASE(ctx, u) \
  Release(((PsanInterceptorContext *) ctx)->thr, pc, u)

#define COMMON_INTERCEPTOR_DIR_ACQUIRE(ctx, path) \
  Acquire(((PsanInterceptorContext *) ctx)->thr, pc, Dir2addr(path))

#define COMMON_INTERCEPTOR_FD_ACQUIRE(ctx, fd) \
  FdAcquire(((PsanInterceptorContext *) ctx)->thr, pc, fd)

#define COMMON_INTERCEPTOR_FD_RELEASE(ctx, fd) \
  FdRelease(((PsanInterceptorContext *) ctx)->thr, pc, fd)

#define COMMON_INTERCEPTOR_FD_ACCESS(ctx, fd) \
  FdAccess(((PsanInterceptorContext *) ctx)->thr, pc, fd)

#define COMMON_INTERCEPTOR_FD_SOCKET_ACCEPT(ctx, fd, newfd) \
  FdSocketAccept(((PsanInterceptorContext *) ctx)->thr, pc, fd, newfd)

#define COMMON_INTERCEPTOR_SET_THREAD_NAME(ctx, name) \
  ThreadSetName(((PsanInterceptorContext *) ctx)->thr, name)

#define COMMON_INTERCEPTOR_SET_PTHREAD_NAME(ctx, thread, name)         \
  if (pthread_equal(pthread_self(), reinterpret_cast<void *>(thread))) \
    COMMON_INTERCEPTOR_SET_THREAD_NAME(ctx, name);                     \
  else                                                                 \
    __psan::ctx->thread_registry.SetThreadNameByUserId(thread, name)

#define COMMON_INTERCEPTOR_BLOCK_REAL(name) BLOCK_REAL(name)

#define COMMON_INTERCEPTOR_ON_EXIT(ctx) \
  OnExit(((PsanInterceptorContext *) ctx)->thr)
...
#define COMMON_INTERCEPTOR_HANDLE_RECVMSG(ctx, msg) \
  HandleRecvmsg(((PsanInterceptorContext *)ctx)->thr, \
      ((PsanInterceptorContext *)ctx)->pc, msg)
#endif

#define COMMON_INTERCEPTOR_GET_TLS_RANGE(begin, end)                           \
  if (PsanThread *t = GetCurrentThread()) {                                    \
    *begin = t->tls_begin();                                                   \
    *end = t->tls_end();                                                       \
  } else {                                                                     \
    *begin = *end = 0;                                                         \
...
#define COMMON_INTERCEPTOR_USER_CALLBACK_START() \
  SCOPED_PSAN_INTERCEPTOR_USER_CALLBACK_START()

#define COMMON_INTERCEPTOR_USER_CALLBACK_END() \
  SCOPED_PSAN_INTERCEPTOR_USER_CALLBACK_END()
...
int sigaction_impl(int sig, const __sanitizer_sigaction *act,
  ...
  // The handler will run synchronously and corrupt psan per-thread state.
...
//    is empty for psan (meant only for msan).
...
PSAN_INTERCEPTOR(void *, __tls_get_addr, void *arg) {
  void *res = REAL(__tls_get_addr)(arg);
  handle_tls_addr(arg, res);
  return res;
}
#else // SANITIZER_S390
PSAN_INTERCEPTOR(uptr, __tls_get_addr_internal, void *arg) {
...
#if SANITIZER_NETBSD
PSAN_INTERCEPTOR(void, _lwp_exit) {
  SCOPED_PSAN_INTERCEPTOR(_lwp_exit);
  DestroyThreadState();
  REAL(_lwp_exit)();
}
#define PSAN_MAYBE_INTERCEPT__LWP_EXIT PSAN_INTERCEPT(_lwp_exit)
#else
#define PSAN_MAYBE_INTERCEPT__LWP_EXIT
#endif

#if SANITIZER_FREEBSD
PSAN_INTERCEPTOR(void, thr_exit, tid_t *state) {
  SCOPED_PSAN_INTERCEPTOR(thr_exit, state);
  DestroyThreadState();
  REAL(thr_exit(state));
}
#define PSAN_MAYBE_INTERCEPT_THR_EXIT PSAN_INTERCEPT(thr_exit)
#else
#define PSAN_MAYBE_INTERCEPT_THR_EXIT
#endif

PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, cond_init, void *c, void *a)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, cond_destroy, void *c)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, cond_signal, void *c)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, cond_broadcast, void *c)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, cond_wait, void *c, void *m)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, mutex_init, void *m, void *a)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, mutex_destroy, void *m)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, mutex_lock, void *m)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, mutex_trylock, void *m)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, mutex_unlock, void *m)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, rwlock_init, void *l, void *a)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, rwlock_destroy, void *l)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, rwlock_rdlock, void *l)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, rwlock_tryrdlock, void *l)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, rwlock_wrlock, void *l)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, rwlock_trywrlock, void *l)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, rwlock_unlock, void *l)
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, once, void *o, void (*i)())
PSAN_INTERCEPTOR_FREEBSD_ALIAS(int, sigmask, int f, void *n, void *o)

PSAN_INTERCEPTOR_NETBSD_ALIAS(int, cond_init, void *c, void *a)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, cond_signal, void *c)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, cond_broadcast, void *c)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, cond_wait, void *c, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, cond_destroy, void *c)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, mutex_init, void *m, void *a)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, mutex_destroy, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, mutex_lock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, mutex_trylock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, mutex_unlock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, rwlock_init, void *m, void *a)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, rwlock_destroy, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, rwlock_rdlock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, rwlock_tryrdlock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, rwlock_wrlock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, rwlock_trywrlock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS(int, rwlock_unlock, void *m)
PSAN_INTERCEPTOR_NETBSD_ALIAS_THR(int, once, void *o, void (*f)())
PSAN_INTERCEPTOR_NETBSD_ALIAS_THR2(int, sigsetmask, sigmask, int a, void *b,
  void *c)
...
static void unreachable() {
  Report("FATAL: PredictiveSanitizer: unreachable called\n");
  Die();
}
#endif
...
#ifdef NEED_TLS_GET_ADDR
#  if !SANITIZER_S390
  PSAN_INTERCEPT(__tls_get_addr);
#  else
  PSAN_INTERCEPT(__tls_get_addr_internal);
  PSAN_INTERCEPT(__tls_get_offset);
#  endif
#endif
  ...
  PSAN_INTERCEPT(longjmp_symname);
  PSAN_INTERCEPT(siglongjmp_symname);
#if SANITIZER_NETBSD
  PSAN_INTERCEPT(_longjmp);
#endif

  PSAN_INTERCEPT(malloc);
  PSAN_INTERCEPT(__libc_memalign);
  PSAN_INTERCEPT(calloc);
  PSAN_INTERCEPT(realloc);
  PSAN_INTERCEPT(reallocarray);
  PSAN_INTERCEPT(free);
  PSAN_INTERCEPT(cfree);
  PSAN_INTERCEPT(munmap);
  PSAN_MAYBE_INTERCEPT_MEMALIGN;
  PSAN_INTERCEPT(valloc);
  PSAN_MAYBE_INTERCEPT_PVALLOC;
  PSAN_INTERCEPT(posix_memalign);

  PSAN_INTERCEPT(strcpy);
  PSAN_INTERCEPT(strncpy);
  PSAN_INTERCEPT(strdup);

  PSAN_INTERCEPT(pthread_create);
  PSAN_INTERCEPT(pthread_join);
  PSAN_INTERCEPT(pthread_detach);
  PSAN_INTERCEPT(pthread_exit);
  #if SANITIZER_LINUX
  PSAN_INTERCEPT(pthread_tryjoin_np);
  PSAN_INTERCEPT(pthread_timedjoin_np);
  #endif

  PSAN_INTERCEPT_VER(pthread_cond_init, PTHREAD_ABI_BASE);
  PSAN_INTERCEPT_VER(pthread_cond_signal, PTHREAD_ABI_BASE);
  PSAN_INTERCEPT_VER(pthread_cond_broadcast, PTHREAD_ABI_BASE);
  PSAN_INTERCEPT_VER(pthread_cond_wait, PTHREAD_ABI_BASE);
  PSAN_INTERCEPT_VER(pthread_cond_timedwait, PTHREAD_ABI_BASE);
  PSAN_INTERCEPT_VER(pthread_cond_destroy, PTHREAD_ABI_BASE);

  PSAN_MAYBE_PTHREAD_COND_CLOCKWAIT;

  PSAN_INTERCEPT(pthread_mutex_init);
  PSAN_INTERCEPT(pthread_mutex_destroy);
  PSAN_INTERCEPT(pthread_mutex_lock);
  PSAN_INTERCEPT(pthread_mutex_trylock);
  PSAN_INTERCEPT(pthread_mutex_timedlock);
  PSAN_INTERCEPT(pthread_mutex_unlock);
#if SANITIZER_LINUX
  PSAN_INTERCEPT(pthread_mutex_clocklock);
#endif
#if SANITIZER_GLIBC
#  if !__GLIBC_PREREQ(2, 34)
  PSAN_INTERCEPT(__pthread_mutex_lock);
  PSAN_INTERCEPT(__pthread_mutex_unlock);
#  endif
#endif

  PSAN_INTERCEPT(pthread_spin_init);
  PSAN_INTERCEPT(pthread_spin_destroy);
  PSAN_INTERCEPT(pthread_spin_lock);
  PSAN_INTERCEPT(pthread_spin_trylock);
  PSAN_INTERCEPT(pthread_spin_unlock);

  PSAN_INTERCEPT(pthread_rwlock_init);
  PSAN_INTERCEPT(pthread_rwlock_destroy);
  PSAN_INTERCEPT(pthread_rwlock_rdlock);
  PSAN_INTERCEPT(pthread_rwlock_tryrdlock);
  PSAN_INTERCEPT(pthread_rwlock_timedrdlock);
  PSAN_INTERCEPT(pthread_rwlock_wrlock);
  PSAN_INTERCEPT(pthread_rwlock_trywrlock);
  PSAN_INTERCEPT(pthread_rwlock_timedwrlock);
  PSAN_INTERCEPT(pthread_rwlock_unlock);

  PSAN_INTERCEPT(pthread_barrier_init);
  PSAN_INTERCEPT(pthread_barrier_destroy);
  PSAN_INTERCEPT(pthread_barrier_wait);

  PSAN_INTERCEPT(pthread_once);

  PSAN_MAYBE_INTERCEPT___FXSTAT;
  PSAN_MAYBE_INTERCEPT_FSTAT;
  PSAN_MAYBE_INTERCEPT_FSTAT64;
  PSAN_INTERCEPT(open);
  PSAN_MAYBE_INTERCEPT_OPEN64;
  PSAN_INTERCEPT(creat);
  PSAN_MAYBE_INTERCEPT_CREAT64;
  PSAN_INTERCEPT(dup);
  PSAN_INTERCEPT(dup2);
  PSAN_INTERCEPT(dup3);
  PSAN_MAYBE_INTERCEPT_EVENTFD;
  PSAN_MAYBE_INTERCEPT_SIGNALFD;
  PSAN_MAYBE_INTERCEPT_INOTIFY_INIT;
  PSAN_MAYBE_INTERCEPT_INOTIFY_INIT1;
  PSAN_INTERCEPT(socket);
  PSAN_INTERCEPT(socketpair);
  PSAN_INTERCEPT(connect);
  PSAN_INTERCEPT(bind);
  PSAN_INTERCEPT(listen);
  PSAN_MAYBE_INTERCEPT_EPOLL;
  PSAN_INTERCEPT(close);
  PSAN_MAYBE_INTERCEPT___CLOSE;
  PSAN_MAYBE_INTERCEPT___RES_ICLOSE;
  PSAN_INTERCEPT(pipe);
  PSAN_INTERCEPT(pipe2);

  PSAN_INTERCEPT(unlink);
  PSAN_INTERCEPT(tmpfile);
  PSAN_MAYBE_INTERCEPT_TMPFILE64;
  PSAN_INTERCEPT(abort);
  PSAN_INTERCEPT(rmdir);
  PSAN_INTERCEPT(closedir);

  PSAN_INTERCEPT(sigsuspend);
  PSAN_INTERCEPT(sigblock);
  PSAN_INTERCEPT(sigsetmask);
  PSAN_INTERCEPT(pthread_sigmask);
  PSAN_INTERCEPT(raise);
  PSAN_INTERCEPT(kill);
  PSAN_INTERCEPT(pthread_kill);
  PSAN_INTERCEPT(sleep);
  PSAN_INTERCEPT(usleep);
  PSAN_INTERCEPT(nanosleep);
  PSAN_INTERCEPT(pause);
  PSAN_INTERCEPT(gettimeofday);
  PSAN_INTERCEPT(getaddrinfo);

  PSAN_INTERCEPT(fork);
  PSAN_INTERCEPT(vfork);
#if SANITIZER_LINUX
  PSAN_INTERCEPT(clone);
#endif
#if !SANITIZER_ANDROID
  PSAN_INTERCEPT(dl_iterate_phdr);
#endif
  PSAN_MAYBE_INTERCEPT_ON_EXIT;
  PSAN_INTERCEPT(__cxa_atexit);
  PSAN_INTERCEPT(_exit);

  PSAN_MAYBE_INTERCEPT__LWP_EXIT;
  PSAN_MAYBE_INTERCEPT_THR_EXIT;
  ...
  if (REAL(__cxa_atexit)(&finalize, 0, 0)) {
    Printf("PredictiveSanitizer: failed to setup atexit callback\n");
    Die();
  }
  if (pthread_atfork(atfork_prepare, atfork_parent, atfork_child)) {
    Printf("PredictiveSanitizer: failed to setup atfork callbacks\n");
    Die();
  }

#if !SANITIZER_APPLE && !SANITIZER_NETBSD && !SANITIZER_FREEBSD
  if (pthread_key_create(&interceptor_ctx()->finalize_key, &thread_finalize)) {
    Printf("PredictiveSanitizer: failed to create thread key\n");
    Die();
  }
#endif

  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(cond_init);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(cond_destroy);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(cond_signal);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(cond_broadcast);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(cond_wait);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(mutex_init);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(mutex_destroy);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(mutex_lock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(mutex_trylock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(mutex_unlock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(rwlock_init);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(rwlock_destroy);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(rwlock_rdlock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(rwlock_tryrdlock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(rwlock_wrlock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(rwlock_trywrlock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(rwlock_unlock);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(once);
  PSAN_MAYBE_INTERCEPT_FREEBSD_ALIAS(sigmask);

  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(cond_init);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(cond_signal);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(cond_broadcast);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(cond_wait);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(cond_destroy);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(mutex_init);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(mutex_destroy);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(mutex_lock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(mutex_trylock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(mutex_unlock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(rwlock_init);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(rwlock_destroy);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(rwlock_rdlock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(rwlock_tryrdlock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(rwlock_wrlock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(rwlock_trywrlock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS(rwlock_unlock);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS_THR(once);
  PSAN_MAYBE_INTERCEPT_NETBSD_ALIAS_THR(sigsetmask);
...
//    output tests.
// 3. Then we switched to atomics+sched_yield. But this produced tons of psan-
...
SANITIZER_INTERFACE_ATTRIBUTE void __psan_testonly_barrier_init(
...
SANITIZER_INTERFACE_ATTRIBUTE void __psan_testonly_barrier_wait(
