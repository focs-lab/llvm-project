...
const char *__psan_default_suppressions() {
  return 0;
}
#endif
...
#if !SANITIZER_GO
  suppression_ctx->Parse(__psan_default_suppressions());
...
static uptr IsSuppressed(const char *stype, const AddressInfo &info,
  ...
  if (suppression_ctx->Match(info.function, stype, sp) ||
    ...
    VPrintf(2, "PredictiveSanitizer: matched suppression '%s'\n", (*sp)->templ);
...
void PrintMatchedSuppressions() {
  ...
  Printf("PredictiveSanitizer: Matched %d suppressions (pid=%d):\n", hit_count,
         (int)internal_getpid());
  for (uptr i = 0; i < matched.size(); i++) {
