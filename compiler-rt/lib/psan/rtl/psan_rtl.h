...
//
// Main internal PSan header file.
//
// Ground rules:
//   - C++ run-time should not be used (static CTORs, RTTI, exceptions, static
//     function-scope locals)
//   - All functions/classes/etc reside in namespace __psan, except for those
//     declared in psan_interface.h.
...
struct AP32 {
  ...
  typedef __psan::MapUnmapCallback MapUnmapCallback;
...
#if defined(PSAN_DEBUG_OUTPUT) && PSAN_DEBUG_OUTPUT >= 1
# define DPrintf Printf
#else
# define DPrintf(...)
#endif

#if defined(PSAN_DEBUG_OUTPUT) && PSAN_DEBUG_OUTPUT >= 2
...
// These need to match __psan_switch_to_fiber_* flags defined in
// psan_interface.h. See documentation there as well.
enum FiberSwitchFlags {
  FiberSwitchFlagNoSync = 1 << 0, // __psan_switch_to_fiber_no_sync
};
...
void LazyInitialize(ThreadState *thr) {
  // If we can use .preinit_array, assume that __psan_init
  // called from .preinit_array initializes runtime before
  // any instrumented code except when psan is used as a 
  // shared library.
