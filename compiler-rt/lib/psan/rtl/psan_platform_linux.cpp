...
static NOINLINE void MapRodata(char* buffer, uptr size) {
  ...
  if (tmpdir == 0)
    return;
  internal_snprintf(buffer, size, "%s/psan.rodata.%d",
...
static void ReExecIfNeeded() {
  ...
  bool reexec = false;
  // PSan doesn't play well with unlimited stack size (as stack
...
#      endif
  ...
  if (reexec) {
    ...
    // this as a last resort (when the memory mapping is incompatible and PSan
    // would fail anyway).
    int old_personality = personality(0xffffffff);
    bool aslr_on =
        (old_personality != -1) && ((old_personality & ADDR_NO_RANDOMIZE) == 0);
...
extern "C" void __psan_tls_initialization() {}
...
void ImitateTlsWrite(ThreadState *thr, uptr tls_addr, uptr tls_size) {
  ...
  const uptr pc = StackTrace::GetNextInstructionPc(
      reinterpret_cast<uptr>(__psan_tls_initialization));
...
// so it must not touch any psan state.
