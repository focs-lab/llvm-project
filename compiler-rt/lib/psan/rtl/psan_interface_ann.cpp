...
INTERFACE_ATTRIBUTE
void __psan_mutex_create(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_create);
  MutexCreate(thr, pc, (uptr)m, flagz & MutexCreationFlagMask);
}

INTERFACE_ATTRIBUTE
void __psan_mutex_destroy(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_destroy);
  MutexDestroy(thr, pc, (uptr)m, flagz);
}

INTERFACE_ATTRIBUTE
void __psan_mutex_pre_lock(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_pre_lock);
...
INTERFACE_ATTRIBUTE
void __psan_mutex_post_lock(void *m, unsigned flagz, int rec) {
  SCOPED_ANNOTATION(__psan_mutex_post_lock);
  ThreadIgnoreSyncEnd(thr);
  ThreadIgnoreEnd(thr);
...
INTERFACE_ATTRIBUTE
int __psan_mutex_pre_unlock(void *m, unsigned flagz) {
  SCOPED_ANNOTATION_RET(__psan_mutex_pre_unlock, 0);
  int ret = 0;
  if (flagz & MutexFlagReadLock) {
...
INTERFACE_ATTRIBUTE
void __psan_mutex_post_unlock(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_post_unlock);
  ThreadIgnoreSyncEnd(thr);
  ThreadIgnoreEnd(thr);
}

INTERFACE_ATTRIBUTE
void __psan_mutex_pre_signal(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_pre_signal);
  ThreadIgnoreBegin(thr, 0);
  ThreadIgnoreSyncBegin(thr, 0);
}

INTERFACE_ATTRIBUTE
void __psan_mutex_post_signal(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_post_signal);
  ThreadIgnoreSyncEnd(thr);
  ThreadIgnoreEnd(thr);
}

INTERFACE_ATTRIBUTE
void __psan_mutex_pre_divert(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_pre_divert);
  // Exit from ignore region started in __psan_mutex_pre_lock/unlock/signal.
  ThreadIgnoreSyncEnd(thr);
  ThreadIgnoreEnd(thr);
}

INTERFACE_ATTRIBUTE
void __psan_mutex_post_divert(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__psan_mutex_post_divert);
  ThreadIgnoreBegin(thr, 0);
  ThreadIgnoreSyncBegin(thr, 0);
}
...
INTERFACE_ATTRIBUTE
void __psan_check_no_mutexes_held() {
  SCOPED_ANNOTATION(__psan_check_no_mutexes_held);
  if (thr->mset.Size() == 0) {
    return;
  }
