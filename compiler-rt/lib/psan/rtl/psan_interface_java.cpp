...
void __psan_java_init(jptr heap_begin, jptr heap_size) {
  JAVA_FUNC_ENTER(__psan_java_init);
  Initialize(thr);
...
int  __psan_java_fini() {
  JAVA_FUNC_ENTER(__psan_java_fini);
  DPrintf("#%d: java_fini()\n", thr->tid);
  DCHECK_NE(jctx, 0);
...
void __psan_java_alloc(jptr ptr, jptr size) {
  JAVA_FUNC_ENTER(__psan_java_alloc);
...
void __psan_java_free(jptr ptr, jptr size) {
  JAVA_FUNC_ENTER(__psan_java_free);
...
void __psan_java_move(jptr src, jptr dst, jptr size) {
  JAVA_FUNC_ENTER(__psan_java_move);
...
jptr __psan_java_find(jptr *from_ptr, jptr to) {
  JAVA_FUNC_ENTER(__psan_java_find);
...
void __psan_java_finalize() {
  JAVA_FUNC_ENTER(__psan_java_finalize);
  DPrintf("#%d: java_finalize()\n", thr->tid);
  AcquireGlobal(thr);
}

void __psan_java_mutex_lock(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_mutex_lock);
...
void __psan_java_mutex_unlock(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_mutex_unlock);
...
void __psan_java_mutex_read_lock(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_mutex_read_lock);
...
void __psan_java_mutex_read_unlock(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_mutex_read_unlock);
...
void __psan_java_mutex_lock_rec(jptr addr, int rec) {
  JAVA_FUNC_ENTER(__psan_java_mutex_lock_rec);
...
int __psan_java_mutex_unlock_rec(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_mutex_unlock_rec);
...
void __psan_java_acquire(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_acquire);
...
void __psan_java_release(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_release);
...
void __psan_java_release_store(jptr addr) {
  JAVA_FUNC_ENTER(__psan_java_release);
