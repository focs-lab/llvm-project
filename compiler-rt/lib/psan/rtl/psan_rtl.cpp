...
//
// Main file (entry points) for the PSan run-time.
//===----------------------------------------------------------------------===//
...
volatile int __psan_resumed = 0;

extern "C" void __psan_resume() {
  __psan_resumed = 1;
}

SANITIZER_WEAK_DEFAULT_IMPL
void __psan_test_only_on_fork() {}
...
static bool InitializeMemoryProfiler() {
  ...
  if (internal_strcmp(fname, "stdout") == 0) {
    ...
    if (ctx->memprof_fd == kInvalidFd) {
      Printf("PredictiveSanitizer: failed to open memory profile file '%s'\n",
             filename.data());
      return false;
    }
  }
  ...
  while (!atomic_load_relaxed(&ctx->stop_background_thread)) {
    ...
    if (flags()->flush_memory_ms > 0) {
      if (last_flush + flags()->flush_memory_ms * kMs2Ns < now) {
        VReport(1, "PredictiveSanitizer: periodic memory flush\n");
        FlushShadowMemory();
    ...
    if (flags()->memory_limit_mb > 0) {
      ...
      VReport(1,
              "PredictiveSanitizer: memory flush check"
              " RSS=%llu LAST=%llu LIMIT=%llu\n",
              (u64)rss >> 20, (u64)last_rss >> 20, (u64)limit >> 20);
      if (2 * rss > limit + last_rss) {
        VReport(1, "PredictiveSanitizer: flushing memory due to RSS\n");
        FlushShadowMemory();
        rss = GetRSS();
        now = NanoTime();
        VReport(1, "PredictiveSanitizer: memory flushed RSS=%llu\n",
                (u64)rss >> 20);
      }
      last_rss = rss;
    }
...
static void PsanOnDeadlySignal(int signo, void *siginfo, void *context) {
...
void Initialize(ThreadState *thr) {
  ...
  ScopedIgnoreInterceptors ignore;
  SanitizerToolName = "PredictiveSanitizer";
  // Install tool-specific callbacks in sanitizer_common.
  SetCheckUnwindCallback(CheckUnwind);

  ctx = new(ctx_placeholder) Context;
  const char *env_name = SANITIZER_GO ? "GORACE" : "PSAN_OPTIONS";
  ...
  __psan::InitializePlatformEarly();
  ...
  InstallDeadlySignalHandlers(PsanOnDeadlySignal);
#endif
  ...
  VPrintf(1, "***** Running under PredictiveSanitizer v3 (pid %d) *****\n",
          (int)internal_getpid());
  ...
  if (flags()->stop_on_start) {
    Printf("PredictiveSanitizer is suspended at startup (pid %d)."
           " Call __psan_resume().\n",
           (int)internal_getpid());
    while (__psan_resumed == 0) {}
  }
...
void MaybeSpawnBackgroundThread() {
  // On MIPS, PSan initialization is run before
...
int Finalize(ThreadState *thr) {
  ...
  if (ctx->nreported) {
    failed = true;
#if !SANITIZER_GO
    Printf("PredictiveSanitizer: reported %d warnings\n", ctx->nreported);
#else
...
void ForkChildAfter(ThreadState* thr, uptr pc, bool start_thread) {
  ...
  VPrintf(1,
          "PredictiveSanitizer: forked new process with pid %d,"
...
uptr __psan_testonly_shadow_stack_current_size() {
  ThreadState *thr = cur_thread();
