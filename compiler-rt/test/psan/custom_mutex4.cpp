// RUN: %clangxx_psan -O1 --std=c++11 %s -o %t && %run %t 2>&1 | FileCheck %s
...
//
// Use after destruction is UB, but __psan_mutex_linker_init and
// __psan_mutex_not_static exist to support global variables of mutex type,
...
int main() {
  ...
  new(&mu1_store) Mutex(false, __psan_mutex_linker_init);
  mu1->Lock();
  mu1->~Mutex();
  mu1->Unlock();
  ...
  new(&mu2_store) Mutex(false, 0, __psan_mutex_not_static);
  mu2->Lock();
  mu2->~Mutex();
  mu2->Unlock();
