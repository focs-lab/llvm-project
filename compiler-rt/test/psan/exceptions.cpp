// RUN: %clangxx_psan -O0 %s -o %t && %run %t 2>&1 | FileCheck %s
...
#define CHECK_SHADOW_STACK(val)                                                \
  fprintf(stderr, (val == __psan_testonly_shadow_stack_current_size()          \
                       ? "OK.\n"                                               \
                       : "Shadow stack leak!\n"));

int main(int argc, const char * argv[]) {
  fprintf(stderr, "Hello, World!\n");
  unsigned long shadow_stack_size = __psan_testonly_shadow_stack_current_size();
