...
int __psan_get_report_tag(void *report, unsigned long *tag);
int __psan_get_report_mop(void *report, unsigned long idx, int *tid, void **addr,
...
void ExternalWrite(void *addr) {
  ...
  __psan_external_write(addr, nullptr, kSwiftAccessRaceTag);
}
...
__psan_on_report(void *report) {
  ...
  unsigned long tag;
  __psan_get_report_tag(report, &tag);
  ...
  void *addr;
  void *trace[16] = {0};
  __psan_get_report_mop(report, /*idx=*/0, &tid, &addr, &size, &write, &atomic,
                        trace, 16);
