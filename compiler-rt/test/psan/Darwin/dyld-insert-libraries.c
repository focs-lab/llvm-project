...
// calls intercepted APIs before and after calling through to the PSan hook.
...
static void pthread_introspection_hook(unsigned int event, pthread_t thread, void *addr, size_t size) {
  ...
  // Use some intercepted APIs *before* PSan hook runs.
  {
    self = pthread_self();
  ...
  // This calls through to the PSan-installed hook, because the injected library
  // constructor (see __library_initializer() below) runs after the PSan
  // initializer.  It replaces and forward to the previously-installed PSan
  ...
  // Use some intercepted APIs *after* PSan hook runs.
  {
