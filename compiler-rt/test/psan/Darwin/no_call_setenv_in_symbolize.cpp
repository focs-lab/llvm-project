...
// present PSan's runtime could derefence bad memory leading to SIGBUS being raised.
// If the signal was caught PSan would deadlock because it would try to run the
// symbolizer again.
// RUN: %env_psan_opts=handle_sigbus=0,symbolize=1 %run %t 2>&1 | FileCheck %s
// RUN: %env_psan_opts=handle_sigbus=0,symbolize=1 __check_mach_ports_lookup=some_value %run %t 2>&1 | FileCheck %s
...
int main() {
  ...
  // Libc will call `realloc()` and PSan's runtime will hit
  // an assertion failure because PSan's runtime uses a different
