// RUN: %clangxx_psan -O1 --std=c++11 %s -o %t
// RUN: %env_psan_opts=report_destroy_locked=0 %run %t 2>&1 | FileCheck %s
...
Mutex mu(false, __psan_mutex_write_reentrant);
long data;

void *thr1(void *arg) {
  mu.Lock();
  mu.~Mutex();
  new(&mu) Mutex(true, __psan_mutex_write_reentrant);
  return 0;
}
