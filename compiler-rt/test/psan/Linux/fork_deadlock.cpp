// RUN: %clangxx_psan -O1 %s -o %t && %env_psan_opts=atexit_sleep_ms=0 %run %t 2>&1 | FileCheck %s
...
// disable_sanitizer_instrumentation on __psan_test_only_on_fork is not
// transitive, so we must apply it here as well.
// Instrumenting alloc_free_blocks() will result in deadlocks in PSan.
...
__psan_test_only_on_fork() {
  const char *msg = "__psan_test_only_on_fork\n";
...
// CHECK: __psan_test_only_on_fork
// CHECK: DONE
