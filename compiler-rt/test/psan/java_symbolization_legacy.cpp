...
extern "C" bool
__psan_symbolize_external(jptr pc, char *func_buf, jptr func_siz,
...
void *Thread(void *p) {
  ...
  __psan_write1_pc((jptr)p, 1234 | kExternalPCBit);
  return 0;
}
...
int main() {
  ...
  __psan_write1_pc((jptr)jheap, 1234 | kExternalPCBit);
  barrier_wait(&barrier);
  pthread_join(th, 0);
