...
// RUN: %env_psan_opts=suppressions='%s.supp' %run %t-dir/executable 2>&1 | FileCheck %s --check-prefix=CHECK-WITHSUPP
...
#ifdef __APPLE__
# define PSAN_MAP_ANON MAP_ANON
#else
# define PSAN_MAP_ANON MAP_ANONYMOUS
#endif
...
void *thr(void *arg) {
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    mmap(0, 4096, PROT_READ, PSAN_MAP_ANON | MAP_PRIVATE | MAP_32BIT, -1 , 0);
    mmap(0, 4096, PROT_WRITE, PSAN_MAP_ANON | MAP_PRIVATE | MAP_32BIT, -1 , 0);
  }
  return 0;
}
