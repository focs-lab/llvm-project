#ifndef __PSAN_TEST_H__
#define __PSAN_TEST_H__
...
#ifndef PSAN_VECTORIZE
#  define PSAN_VECTORIZE __SSE4_2__
#endif
...
// PSan-invisible barrier.
// Tests use it to establish necessary execution order in a way that does not
// interfere with psan (does not establish synchronization between threads).
typedef unsigned invisible_barrier_t;

#ifdef __cplusplus
extern "C" {
#endif
void __psan_testonly_barrier_init(invisible_barrier_t *barrier,
    unsigned count);
void __psan_testonly_barrier_wait(invisible_barrier_t *barrier);
unsigned long __psan_testonly_shadow_stack_current_size();
#ifdef __cplusplus
}
#endif

static inline void barrier_init(invisible_barrier_t *barrier, unsigned count) {
  __psan_testonly_barrier_init(barrier, count);
}

static inline void barrier_wait(invisible_barrier_t *barrier) {
  __psan_testonly_barrier_wait(barrier);
}
