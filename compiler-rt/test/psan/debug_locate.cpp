...
extern "C" const char *
__psan_locate_address(void *addr, char *name, size_t name_size,
...
int main() {
  ...
  void *start;
  size_t size;
  type = __psan_locate_address(&stack_var, name, 128, &start, &size);
  fprintf(stderr, "type: %s\n", type);
  // CHECK: type: stack

  type = __psan_locate_address(&global_var, name, 128, &start, &size);
  fprintf(stderr, "type: %s, name = %s, start = %p, size = %zu\n", type, name,
          start, size);
  // CHECK: type: global, name = global_var, start = [[GLOBAL_VAR]], size = {{8|0}}

  type = __psan_locate_address(heap_var, name, 128, &start, &size);
