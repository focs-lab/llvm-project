// RUN: %clangxx_psan %s -o %t -DLockType=PthreadMutex
// RUN: %deflake %run %t | FileCheck %s --check-prefix=CHECK --check-prefix=CHECK-NOT-SECOND
// RUN: %env_psan_opts=second_deadlock_stack=1 %deflake %run %t | FileCheck %s --check-prefix=CHECK --check-prefix=CHECK-SECOND
// RUN: %clangxx_psan %s -o %t -DLockType=PthreadSpinLock
// RUN: %deflake %run %t | FileCheck %s
// RUN: %clangxx_psan %s -o %t -DLockType=PthreadRWLock
// RUN: %deflake %run %t | FileCheck %s --check-prefix=CHECK --check-prefix=CHECK-RD
// RUN: %clangxx_psan %s -o %t -DLockType=PthreadRecursiveMutex
