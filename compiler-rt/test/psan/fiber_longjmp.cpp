...
void switch0() {
  if (!sigsetjmp(jmpbuf, 0)) {
    __psan_switch_to_fiber(orig_fiber[0], 0);
...
void func() {
  ...
  for (;;) {
    switch0();
    if (!sigsetjmp(jmpbuf, 0)) {
      __psan_switch_to_fiber(orig_fiber[1], 0);
...
void *Thread(void *x) {
  orig_fiber[1] = __psan_get_current_fiber();
  for (unsigned i = 0; i < N; i++) {
    barrier_wait(&barrier);
    if (!sigsetjmp(orig_jmpbuf[1], 0)) {
      __psan_switch_to_fiber(fiber, 0);
      siglongjmp(jmpbuf, 1);
    }
    barrier_wait(&barrier);
  }
...
int main() {
  ...
  orig_fiber[0] = __psan_get_current_fiber();
