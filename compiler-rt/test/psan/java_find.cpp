...
static void verify_find(jptr from, jptr to, jptr expected_addr,
                        jptr expected_size) {
  jptr addr = from;
  jptr size = __psan_java_find(&addr, to);
  if (expected_size) {
    if (!size) {
...
int main() {
  ...
  const int size1 = 16;
  __psan_java_alloc(jheap, size1);

  const jptr addr2 = addr1 + size1;
  const int size2 = 32;
  __psan_java_alloc(jheap + size1, size2);

  const jptr addr3 = addr2 + size2;
  const int size3 = 1024;
  __psan_java_alloc(jheap + size1 + size2, size3);
  ...
  __psan_java_move(addr2, addr4, size2);
