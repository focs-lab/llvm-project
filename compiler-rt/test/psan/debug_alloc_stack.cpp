...
// RUN: env %env_psan_opts=stack_trace_format=DEFAULT %deflake %run %t 2>&1 | FileCheck %s
...
extern "C" int __psan_get_alloc_stack(void *addr, void **trace, size_t size,
...
int main() {
  ...
  num_frames =
      __psan_get_alloc_stack(mem, trace, num_frames, &thread_id, &thread_os_id);
