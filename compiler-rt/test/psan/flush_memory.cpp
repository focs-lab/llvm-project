...
// RUN: %env_psan_opts=flush_memory_ms=1:flush_symbolizer_ms=1:memory_limit_mb=1 not %run %t 2>&1 | FileCheck %s
#include "test.h"
...
int main() {
  ...
  // (if the state flush happens between racing accesses, psan won't
