// RUN: %clang_psan %darwin_min_target_with_tls_support %s -o %t
// RUN: %clang_psan %darwin_min_target_with_tls_support %s -DBUILD_SO -fPIC -o \
// RUN:   %t-so.so -shared
// RUN: %run %t 2>&1 | FileCheck %s
// XFAIL: target={{.*netbsd.*}}

// Test that psan cleans up dynamic TLS memory between reuse.
